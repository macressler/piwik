    ...
    public function getMigrations(Updater $updater)
    {
        ...
        foreach ($tables as $tableName) {
            if (strpos($tableName, 'archive_') !== false) {
                $sql[] = $this->migration->db->sql('UPDATE `' . $tableName . '` SET `name`=REPLACE(`name`, \'Referers_\', \'Referrers_\') WHERE `name` LIKE \'Referers_%\'');
            }
        }
        $errorCodeTableNotFound = '1146';

        // Rename custom segments containing Referers segments
        $sql[] = $this->migration->db->sql('UPDATE `' . Common::prefixTable('segment') . '` SET `definition`=REPLACE(`definition`, \'referer\', \'referrer\') WHERE `definition` LIKE \'%referer%\'', $errorCodeTableNotFound);
        ...
        $sql[] = $this->migration->db->sql($query, $errorCodeTableNotFound);
        ...
        $sql[] = $this->migration->db->sql($query, Updater\Migration\Db::ERROR_CODE_DUPLICATE_ENTRY); // http://forum.piwik.org/read.php?2,106895

        $sql[] = $this->migration->plugin->activate('Referrers');
        $sql[] = $this->migration->plugin->activate('ScheduledReports');
